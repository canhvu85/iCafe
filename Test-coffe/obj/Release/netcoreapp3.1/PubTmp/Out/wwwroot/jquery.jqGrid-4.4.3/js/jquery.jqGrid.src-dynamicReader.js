		...
		return this.each(function () {
			...
			var sortkeys = ["shiftKey", "altKey", "ctrlKey"],
				...
				addJSONData = function (data, t, rcnt, more, adjust) {
					...
					var self = $(ts), ir = 0, v, i, j, f = [], F, arrayReader, objectReader, rowReader, cur, gi = ts.p.multiselect ? 1 : 0, si = 0, addSubGridCell, ni = ts.p.rownumbers === true ? 1 : 0, len, drows, idn, rd = {}, fpos, idr, rowData = [], cn = (ts.p.altRows === true) ? " " + ts.p.altclass : "", cn1, lp;
					...
					while (i < len) {
						...
						if (idr === undefined) {
							if (typeof idn === "number" && ts.p.colModel[idn] != null) {
								// reread id by name
								idr = $.jgrid.getAccessor(cur, ts.p.colModel[idn].name);
							}
							if (idr === undefined) {
								idr = br + i;
								if (f.length === 0) {
